...
def Integrate(w         : Weight_Function,
    ...
    partitioned \Omega using a regular grid (for each dimension, the grid lines
    along that dimension are evenly spaced. Girdlines along different
    dimensions, however, may have different spacing between them). Under this
    assumption, the grid partitions Omega into a set of smaller rectangles.
    Since the gird is uniform, each sub-rectangle has the same volume. Let V
    denote that volume. Further, let {X_1, ... , X_N} denote the set of grid
    points. Since w is, by assumption, zero along the boundary of Omega, it must
    be zero at each gridpoint on the boundary of Omega. With this assumption,
    it can be shown that the composite trapezodial rule apprpxoimation to the
    ...
    calculated D(w) (via the "Add_Derivative" method) on the integration grid.

    D : This is a derivative operator.

    FU_Grid : We assume this is a 1D tensor whose ith entry holds the value of a
    trial function, F(U), at the ith grid point. That is,
        FU_Grid[i] = F(U(X_i)).
    We assume that the user evaluated FU_Grid along the same grid used to
    ...
    # First, determine the grid-point indices (subset of {1, 2, ... , N}) in the
    # support of w.
    Supported_Indices : torch.Tensor = w.Supported_Indices;

    # Next, extract the corresponding elements of FU_Grid.
    FU_Grid_Supported : torch.Tensor = FU_Grid[Supported_Indices];

    # Get D(w) evaluated on the grid points in the support of w.
    Dw_Grid_Supported : torch.Tensor = w.Derivatives[tuple(D.Encoding)];

    # Sum the element-wise product of FU_Grid_Supported and D(w). This yields
    # the summation \sum_{i = 1}^{N} D w(X_i) F(U(X_i)).
    Sum : torch.Tensor = torch.sum(torch.multiply(FU_Grid_Supported, Dw_Grid_Supported));
