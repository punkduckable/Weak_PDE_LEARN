...
def Plot_Losses(Save_File_Name      : str,
    ...
    Weak_DF : pandas.DataFrame = Make_Test_Train_DataFrame(
                                    Train_Losses        = Train_Weak_Losses,
                                    Test_Losses         = Test_Weak_Losses,
                                    Labels              = Labels);
    ...
    # Make a figure for the history of the Weak Form loss.
    plt.figure(2);
    plt.clf();
    seaborn.lineplot(data = Weak_DF, x = "Epoch Number", y = "Losses", hue = "Experiment", style = "Train/Test", palette = palette);
    plt.title("Weak Form Loss");
    plt.xlabel("epoch number");
    plt.ylabel("Loss");
    plt.yscale('log');
    plt.legend();
