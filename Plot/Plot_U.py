...
def Plot_U( Load_File_Name          : str,
    ...
    LHS_Term        : Library_Term          = Build_Library_Term_From_State(State = Saved_State["LHS Term State"]);
    ...
    ############################################################################
    # Evaluate each U, PDE residual on the inputs.
    ...
    for k in range(Num_DataSets):
        ...
        for i in range(Num_Batches):
            kth_Residual_Coords[i*1000 : (i + 1)*1000] = PDE_Residual(
                                                        U           = U_List[k],
                                                        Xi          = Xi,
                                                        Mask        = Mask,
        ...
        if(Num_Coords % 1000 != 0):
            kth_Residual_Coords[Num_Batches*1000:] = PDE_Residual(
                                                    U           = U_List[k],
                                                    Xi          = Xi,
                                                    Mask        = Mask,
